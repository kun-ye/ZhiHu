...
public class CartoonFragment extends BaseFragment {
    ...
    public View onCreateSuccessView() {
        ...
        editor_image5.setOutlineProvider(viewOutlineProvider);
        editor_image5.setClipToOutline(true);

        view.addHeaderView(header,data,false);
        view.setAdapter(new CartoonAdapter(data));
    ...
    public LoadingPage.ResultState onLoad() {
        ...
        data = protocol.getData(GlobalConstants.CARTOON_URL);
        editors = protocol.getEditors();
        bored = protocol.getBored();
        return check(data);
    }

    class CartoonAdapter extends MyBaseAdapter<BoredBean.StoriesBean> {

        public CartoonAdapter(List<BoredBean.StoriesBean> data) {
            super(data);
        }
